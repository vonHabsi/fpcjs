...
const
  {$IFDEF LINUX}
    LibName='libmozjs.so';
  {$ELSE}                         //15R
    {$IFNDEF JSDebug}LibName = 'js3250.dll'{$ELSE}LibName = 'js3215D.dll'{$ENDIF};
  {$ENDIF}
...
{$ifdef FPC}
// added for compatibility with fpc
...
const
  ...
  other_class: JSClass = (name: 'other'; flags: JSCLASS_HAS_PRIVATE; addProperty: JS_PropertyStub;
                           delProperty: JS_PropertyStub; getProperty: JS_PropertyStub; setProperty: JS_PropertyStub;
                           enumerate: JS_EnumerateStub; resolve: JS_ResolveStub; convert: JS_ConvertStub;
                           finalize: JS_FinalizeStub );
{$endif}			  
...
uses {$IFDEF LINUX}Math, {$ENDIF} Classes;

{function JS_EnumerateStub2(cx: PJSContext; obj: PJSObject): JSBool; cdecl; 
begin
  writeln('FFFFFFFFFf');
end;

function JS_GlobalClassMethodCall(cx: PJSContext; obj: PJSObject; argc: uintN; argv: pjsval; rval: pjsval): JSBool; cdecl;
begin
  writeln('ZZZZZZZZZZ');
end;}
...
{$ELSE}
  {$IFNDEF JSUnicode}
    ...
    function CreateWideString(const Text: String): PWideChar; overload;
    begin
      Result := PWideChar{StringToOleStr}(Text);
    end;
  {$ELSE}
    function CreateAnsiString(const Text: WideString): PChar; overload;
    begin
